class level3 extends Phaser.Scene {
  ...
  preload() {
    // Step 1, load JSON
    this.load.tilemapTiledJSON("world3", "assets/Map_3.tmj");

    // Step 2 : Preload any images here
    this.load.image("StadiumIMG", "assets/1_Terrains_and_Fences_32x32.png");
  ...
  this.anims.create({
    ...
    let stadiumtiles = map.addTilesetImage(
      "1_Terrains_and_Fences_32x32",
      "StadiumIMG"
    );

    //Step 5  create an array of tiles
    let tilesArray = [stadiumtiles];

    // Step 6  Load in layers by layers
    this.floorLayer1 = map.createLayer("floorLayer1", tilesArray, 0, 0);
    this.floorLayer2 = map.createLayer("floorLayer2", tilesArray, 0, 0);
    this.floorLayer3 = map.createLayer("floorLayer3", tilesArray, 0, 0);
    this.props = map.createLayer("props", tilesArray, 0, 0);
...
handlePaper3Overlap(player, paper3) {
  ...
  update() {
    ...
    if (this.papersCollected.paper1 && this.papersCollected.paper2) {
      // Check for teleportation to goalScene only if both papers are collected
      if (
          this.player.x > 1413 &&
          this.player.x < 1506 &&
          this.player.y < 711 &&
          ...
          this.goalScene(); // Call the goalScene function within the scene context
      }
    }
  ...
  // Function to teleport to goalScene
goalScene() {
  console.log("Teleporting to goalScene");
